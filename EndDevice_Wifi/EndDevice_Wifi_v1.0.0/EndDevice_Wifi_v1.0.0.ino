...
struct StructParam {
  ...
  const char _Version[30] =           "EndDevice_Esp8266 1.0.0";    //版本信息
  ...
  char _SERVER_IP[30] =           "10.70.21.92";                    //服务器地址
...
void AT_Commands(String item, String value) {
  ...
  if (item.equals("AT+CTSERVER")) {                         //连接服务端
    if (ParameterList._SERVER_TYPE)
      connectToServer("UDP", ParameterList._SERVER_IP, ParameterList._SERVER_PORT);
    else
      connectToServer("TCP", ParameterList._SERVER_IP, ParameterList._SERVER_PORT);
    Serial.print("OK");
    Serial.print(_Terminator);
    return;
  }
