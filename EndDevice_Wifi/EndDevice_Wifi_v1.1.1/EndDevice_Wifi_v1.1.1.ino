/*
  #更新说明 v1.1.1
  1.新增功能:与服务器连接断开后自动重连;
  2.新增AT指令"AT+SERVERAUTOCT=<0/1>"用以开关与服务器通信断开后重连功能;
  3.开机后自动与服务器建立连接;
  4.格式化输出函数可将数据转发至软件串口、硬件串口、Socket通信;
  5.软串口转发标记变量"boolean _SS_ECHO"改为"short_SS_ECHO";
...
struct StructParam {
  ...
  const char    _Version[30] =        "EndDevice_Esp8266 1.1.1";        //版本信息
  const char    _Date[11] =           "2016-05-06";                     //烧录时间
  /* - - - - - - - - WIFI模块成员变量 - - - - - - - - - -*/
...
void AT_Commands(String item, String value, int PRINT_MODE) {
  ...
  if (item.equals("AT+SERVERAUTOCT")) {                        //设置服务端连接类型
    if (!value.equals("")) {
