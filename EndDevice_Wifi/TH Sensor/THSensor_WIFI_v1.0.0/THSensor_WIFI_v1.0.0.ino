...
/* - - - - - - - - 自定义宏定义 - - - - - - - - - -*/
#define DTH11_PIN                      3                        //DTH11温湿度模块的数据引脚
/* - - - - - - - - - - - - - - - - - - - - - - - - */
...
struct StructParam {
  ...
  unsigned int _ALARM_INTERVAL =      30000;                            //警报发送间隔时间
  unsigned short _HUM_ALARMVALUE =    50;                               //湿度警报线
  unsigned short _TEMP_ALARMVALUE =   50;                               //温度警报线
  /* - - - - - - - - - - - - - - - - - - - - - - - - - */
} ParameterList;
...
void loop() {
  ...
  if (!_BoolAT) {
    ...
    HT_Warning(ParameterList._ALARM_INTERVAL);
  }
...
/**
   温湿度警戒值报警
*/
void HT_Warning(const unsigned long Interval) {
  /*— — — — — — — — — ↓ 时间控制 请勿改动 ↓ — — — — — — — — — — */
  ...
  /*代码区域:开始*/
  int chk = DHT11.read(DTH11_PIN);
  String data = "WARNING ";
  bool s = 0;
  if ((float)DHT11.humidity > ParameterList._HUM_ALARMVALUE) {
    data += "HUM:";
    data += String(ParameterList._HUM_ALARMVALUE);
    data += " ";
    s = 1;
  }
  if ((float)DHT11.temperature > ParameterList._TEMP_ALARMVALUE) {
    data += "TEMP:";
...
void AT_Commands(String item, String value, int PRINT_MODE) {
  ...
  if (item.equals("AT+GETTH")) {                              //获取当前温湿度的数值
    int chk = DHT11.read(DTH11_PIN);
    String data = "HT ";
    data += (float)DHT11.humidity;    //获取湿度
    data += " ";
    data += (float)DHT11.temperature; //获取温度(摄氏度)
    printline(data, PRINT_MODE);
    return;
  }

  if (item.equals("AT+ALARMVALUE")) {                             //设置温湿度警戒值
    if (!value.equals("")) {
      ...
      int hum = value.substring(0, sp).toInt();
      int temp = value.substring(sp + 1).toInt();
      ParameterList._TEMP_ALARMVALUE = temp;
      ParameterList._HUM_ALARMVALUE = hum;
      saveParameterBean();
      printline("OK", PRINT_MODE);
    } else {
      printline(String(ParameterList._HUM_ALARMVALUE) + "," + String(ParameterList._TEMP_ALARMVALUE), PRINT_MODE);
    }
    return;
  }
