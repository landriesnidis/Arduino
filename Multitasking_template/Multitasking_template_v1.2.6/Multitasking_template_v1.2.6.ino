/*
   #更新说明 v1.2.6
   1.新增动态设置模拟引脚监听功能,通过AT指令配置需监听的模拟引脚取值范围,当超出该范围触发对应的数字引脚.
...
struct StructParam {
  ...
  const char _Version[30] =           "Multitasking_template 1.2.6";//版本信息
  const char _Date[11] =              "2016-05-05";                 //烧录时间
  int AnalogPinListerenrConfig[6][5];                               //模拟引脚监听配置
  /* - - - - - - - - 自定义成员变量 - - - - - - - - - -*/
...
/**
   模拟引脚电压值监控函数
*/
void AnalogListener(const unsigned long Interval) {
  static unsigned int freq = 0;
  if (++freq * ParameterList._LoopCycle < Interval) {
    return;
  } else {
  ...
  int sign = 0;
  for (i = 0; i < 6; i++) {
    sign = ParameterList.AnalogPinListerenrConfig[i][0];
    if (sign) {
      AnalogPinListener(ParameterList.AnalogPinListerenrConfig[i][1], ParameterList.AnalogPinListerenrConfig[i][2], ParameterList.AnalogPinListerenrConfig[i][3], ParameterList.AnalogPinListerenrConfig[i][4]);
    }
  }
}
...
void AT_Commands(String item, String value) {
  ...
  if (item.equals("AT+APLSNCFG")) {                        //设置模拟值引脚监听配置
    //示例
    //AT+APLSNCFG       ……………………  查询所有监听配置
    //AT+APLSNCFG=1     ……………………  对a1引脚进行配置
    if (!value.equals("")) {
      ParameterList.AnalogPinListerenrConfig[value.toInt()][0] = ReceiveLine("Set Work Mode:").toInt();
      ParameterList.AnalogPinListerenrConfig[value.toInt()][1] = value.toInt();
      ParameterList.AnalogPinListerenrConfig[value.toInt()][2] = ReceiveLine("Input Max Value:").toInt();
      ParameterList.AnalogPinListerenrConfig[value.toInt()][3] = ReceiveLine("Input Min Value:").toInt();
      ParameterList.AnalogPinListerenrConfig[value.toInt()][4] = ReceiveLine("Input Data Pin:").toInt();
      saveParameterBean();
      Serial.print("OK");
    } else {
      for (int i = 0; i < 6; i++) {
        if (ParameterList.AnalogPinListerenrConfig[i][0] == 1) {
          Serial.print("APin:");
          Serial.print(ParameterList.AnalogPinListerenrConfig[i][1]);
          Serial.print("\tMax:");
          Serial.print(ParameterList.AnalogPinListerenrConfig[i][2]);
          Serial.print("\tMin:");
          Serial.print(ParameterList.AnalogPinListerenrConfig[i][3]);
          Serial.print("\tDPin:");
          Serial.print(ParameterList.AnalogPinListerenrConfig[i][4]);
        }
      }
    }
    Serial.print(_Terminator);
...
/**
  模拟引脚电压值监控,取值范围判断与触发
*/
void AnalogPinListener(int anPin, int maxValue, int minValue, int daPin) {
  int value = analogRead(anPin);
  if (value <= maxValue || value < minValue) {
    digitalWrite(daPin, HIGH);
  } else {
    digitalWrite(daPin, LOW);
  }
}
